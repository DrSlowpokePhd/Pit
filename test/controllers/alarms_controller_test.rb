...
class AlarmsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create alarm" do
    assert_difference('Alarm.count') do
      post alarms_url, params: { alarm: { name: @alarm.name, time: @alarm.time, user: @alarm.user } }
    end
  ...
  test "should update alarm" do
    patch alarm_url(@alarm), params: { alarm: { name: @alarm.name, time: @alarm.time, user: @alarm.user } }
